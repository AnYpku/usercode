...
{
  //TFile* f1 = new TFile("../WGammaOutput/ELECTRON_WGamma/PreliminaryForTemplateMethodSelected/selected_WGamma_blindCOMBINED_DATA.root");
  TFile* f1 = new TFile("../WGammaOutput/ELECTRON_WGamma/FullySelected/selected_WGamma_UNblind_DATA.root");
...
//  TTree* tr2 = (TTree*)f2->Get("miniTree");
  ...
  tr2->ReadFile("../WGammaOutput/Lovedeep/outputData2012a.txt","run2/I:event2/L:WMt2/F:phoEt2/F:phoSCEta2/F:Meg2/F");

  int ev1;
  Long64_t ev2;
  int ipho1, ipho2, ilep11, ilep12;
  float phoEt1, phoEt2, phoSCEta1, WMt1, WMt2, pfMET_NS1, pfMETPhi_NS1, lepPt1, lepPhi1, phoSigmaIEtaIEta1, run2;
  TBranch *b_ev1, *b_ev2, *b_ipho1, *b_ipho2, *b_ilep11, *b_ilep12, *b_phoEt1, *b_phoEt2, *b_phoSCEta1, *b_lepPt1, *b_lepPhi1, *b_run2;
  TBranch *b_WMt1, *b_WMt2, *b_pfMET_NS1, *b_pfMETPhi_NS1, *b_phoSigmaIEtaIEta1;
  ...
  tr2->SetBranchAddress("run2", &run2, &b_run2);
...
//  tr2->SetBranchAddress("pho_pt",&phoEt2, &b_phoEt2);
  ...
  TCut cutPt="phoEt>15";
  TCut cutB="phoSCEta>-1.4442 && phoSCEta<1.4442 && phoSigmaIEtaIEta<0.011";
  TCut cutE="((phoSCEta>-2.5 && phoSCEta<-1.566)||(phoSCEta>1.566 && phoSCEta<2.5)) && phoSigmaIEtaIEta<0.033";
  long nentries1 = tr1->GetEntries(cutPt && (cutB || cutE));
...
//  nentries1=10000;
  for (long ient1=0; ient1<nentries1; ient1++){
    ...
    b_phoSCEta1->GetEntry(ient1);
      ...
      if (phoSCEta1>-1.4442 && phoSCEta1<1.4442 && phoSigmaIEtaIEta1>0.011) continue;
      if (((phoSCEta1>-2.5 && phoSCEta1<-1.566)||(phoSCEta1>1.566 && phoSCEta1<2.5)) && phoSigmaIEtaIEta1>0.033) continue;
...
//    b_ilep11->GetEntry(ient1);
    ...
    for (long ient2=0; ient2<nentries2; ient2++){
      b_run2->GetEntry(ient2);
      if (run2>193621) break;
...
//      b_ilep12->GetEntry(ient2);
  ...
  nentries1 = tr1->GetEntries(cutPt && (cutB || cutE));
  nentries2 = tr2->GetEntries();

  std::cout<<"nentries1="<<nentries1<<", nentries2="<<nentries2<<std::endl;

  nentries1 = tr1->GetEntries();
//  nentries2=10000;//start from 2137 - all events
  //2136 - 0 ev
...
//      b_ilep11->GetEntry(ient1);
    ...
    if (evMatched==0){
      std::cout<<"event=="<<ev2_Ref<<", run2="<<run2<<std::endl;
    }
  }//end of loop over ient2

}// end of CompareTwoSelectedTrees
