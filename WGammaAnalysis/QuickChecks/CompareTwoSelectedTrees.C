...
void CompareTwoSelectedTrees()
{
  ...
  int ev1, ev2, inpFN1, inpFN2, ipho1, ipho2, ilep11, ilep12;
  TBranch *b_ev1, *b_ev2, *b_inpFN1, *b_inpFN2, *b_ipho1, *b_ipho2, *b_ilep11, *b_ilep12;

  tr1->SetBranchAddress("event", &ev1, &b_ev1);
  tr1->SetBranchAddress("inputFileNumber", &inpFN1, &b_inpFN1);
  tr1->SetBranchAddress("ipho", &ipho1, &b_ipho1);
  tr1->SetBranchAddress("ilep1", &ilep11, &b_ilep11);
  tr2->SetBranchAddress("event", &ev2, &b_ev2);
  tr2->SetBranchAddress("inputFileNumber", &inpFN2, &b_inpFN2);
  tr1->SetBranchAddress("ipho", &ipho2, &b_ipho2);
  tr1->SetBranchAddress("ilep1", &ilep12, &b_ilep12);

  long nentries1 = 100; //tr1->GetEntries();
  long nentries2 = 100; //tr2->GetEntries();

  std::cout<<"Found in tree 1 only:"<<std::endl;
  for (long ient1=0; ient1<nentries1; ient1++){
    b_inpFN1->GetEntry(ient1);
    if (inpFN1!=0) break;
    ...
    for (long ient2=0; ient2<nentries2; ient2++){
      b_inpFN2->GetEntry(ient2);
      if (inpFN2!=0) break;
      b_ev2->GetEntry(ient2);
      b_ipho2->GetEntry(ient2);
      b_ilep12->GetEntry(ient2);
      if (ev1==ev2 && ipho1==ipho2 && ilep11==ilep12) {evMatched=1;break;}
