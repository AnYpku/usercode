...
Unfolding::Unfolding(int channel, string configFile, bool isPuReweight, bool isDebugMode)
{
...
void Unfolding::LoopOverInputFiles()
{
       ...
       for (int inputFileN=0; inputFileN< inputFileNMax; inputFileN++)
         {
           ...
           if (f.IsOpen()) 
             ...
             {
                std::cout<<"ERROR detected in Unfolding::LoopOverInputFiles: file "<<INPUT_->allInputs_[iSource].fileNames_[inputFileN]<<" was not found"<<std::endl;
                return;
             } 
           f.cd("ggNtuplizer");
           ...
           if (tree) 
             ...
             {
                std::cout<<"ERROR detected in Unfolding::LoopOverInputFiles: tree in the file "<<INPUT_->allInputs_[iSource].fileNames_[inputFileN]<<" does not exist"<<std::endl;
                return;
             }  
           ...
           if (isSkimmed){
              std::cout<<"ERROR detected in Unfolding::LoopOverInputFiles: file"<<INPUT_->allInputs_[iSource].fileNames_[inputFileN]<<" is skimmed; skimmed signal MC file can't be used for acceptance and efficiency calculation; please, use the full signal MC file"<<std::endl;
              return;
           }
...
//           }
           ...
           LoopOverTreeEvents();
             //method of this class (Unfolding)
           fChain=0;
             //field of TEventTree
...
void Unfolding::LoopOverTreeEvents()
{
   if (fChain == 0) return;
   ...
   //loop over events in the tree
     {
        ...
        if (channel_==TConfiguration::MUON) nLe_=treeLeaf.nMu;
          ...
          {
             std::cout<<"Error detected in  Unfolding::LoopOverTreeEvents: channel must be either MUON or ELECTRON."<<std::cout;
             return;
          }
...
void Unfolding::PlotAndSaveOutput()
{
