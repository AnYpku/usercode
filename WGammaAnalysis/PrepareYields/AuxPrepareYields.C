...
void AuxPrepareYieldsCommon(TPrepareYields& prep, TPrepareYields::PrepareYieldsPars& pars, int channel, int vgamma, int blind, TString varKin, int nKinBins, float* kinBinLims);
...
void AuxPrepareYields(int channel, int vgamma, int blind, TString varKin, int nKinBins, float* kinBinLims)
{
  ...
  AuxPrepareYieldsCommon(prep, pars, channel, vgamma, blind, varKin, nKinBins, kinBinLims);

  prep.PlotPrintSave();
  
}

void AuxSubtractBackground(int channel, int vgamma, int blind, TString varKin, int nKinBins, float* kinBinLims)
{
  ...
  prep.SetYieldsDataDrivenTrue("DDtrue", "DD true", 4, fileName, strYields1DTrue, strYieldTotTrue);
  prep.SetYieldsDataDrivenFake("DDfake", "DD fake", 2, fileName, strYields1DFake, strYieldTotFake);
...
void AuxPrepareYieldsCommon(TPrepareYields& prep, TPrepareYields::PrepareYieldsPars& pars, int channel, int vgamma, int blind, TString varKin, int nKinBins, float* kinBinLims)
{
  ...
  if (vgamma==config.W_GAMMA && channel==config.ELECTRON)
    prep.SetOneYieldSource(prep.SIGMC, "Wg_to_enu", "W#gamma#rightarrowe#nu#gamma", 634, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.SIGMC), "selectedEvents");
  ...
  // bkg MC  
  prep.SetOneYieldSource(prep.BKGMC_FAKE, "Wjets_to_lnu", "W+jets#rightarrowl#nu+jets", 433, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.BKGMC,"Wjets_to_lnu"), "selectedEvents");
  
  prep.SetOneYieldSource(prep.BKGMC_FAKE, "DYjets_to_ll", "DY+jets#rightarrowl#bar{l}", 418, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.BKGMC,"DYjets_to_ll"), "selectedEvents");
  
  prep.SetOneYieldSource(prep.BKGMC_TRUE, "Zg", "Z#gamma#rightarrowl#bar{l}", 879, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.BKGMC,"Zg"), "selectedEvents");

  prep.SetOneYieldSource(prep.BKGMC_FAKE, "ttbarjets", "t#bar{t}+jets", 631, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.BKGMC,"ttbarjets") , "selectedEvents");

  prep.SetOneYieldSource(prep.BKGMC_TRUE, "Wg_to_taunu", " W#gamma#rightarrow#tau#nu#gamma", 401, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.BKGMC,"Wg_to_taunu") , "selectedEvents");

  prep.SetOneYieldSource(prep.BKGMC_TRUE, "WWg", " WW#gamma", 884, config.GetSelectedName(config.FULLY,channel,vgamma,blind,config.BKGMC,"WWg") , "selectedEvents");
